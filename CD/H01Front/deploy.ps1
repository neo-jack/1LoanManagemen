# Deploy script for H01Front microfront-end with independent port deployment
Write-Host "[Deploy] Starting H01Front microfront-end deployment process..." -ForegroundColor Green

# Load environment variables from .env file
$envFile = Join-Path $PSScriptRoot ".env-h01front"
...
$serverPath = if ($env:SERVER_H01FRONT_PATH) { $env:SERVER_H01FRONT_PATH } else { "/root/project/1LoanManagement/H01Front/dist" }
$serverConfigPath = if ($env:SERVER_CONFIG_PATH) { $env:SERVER_CONFIG_PATH } else { "/root/project/CD/1LoanManagement/H01Front" }
$projectPath = if ($env:LOCAL_H01FRONT_PATH) { $env:LOCAL_H01FRONT_PATH } else { "C:\Users\what\Desktop\my-project\1LoanManagement\H01Front" }
...
Write-Host "[Config] Target: H01Front Microfront-end (Port 3001)" -ForegroundColor Yellow
Write-Host "[Config] Project Path: $projectPath" -ForegroundColor Yellow

# Check and navigate to H01Front
if (!(Test-Path $projectPath)) {
    Write-Host "[Error] H01Front directory not found at: $projectPath" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    # 2. Build project
    Write-Host "[Step2] Building H01Front project..." -ForegroundColor Cyan
    npm run build
    if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "[Step4] Uploading H01Front dist to server..." -ForegroundColor Cyan
    ...
    Write-Host "[Step5] Uploading H01Front config files..." -ForegroundColor Cyan
    ...
    Write-Host "[Step6] Stopping existing H01Front container..." -ForegroundColor Cyan
    try {
        if ($plinkExists) {
            $plinkCommand = "plink -batch -pw '$serverPassword' ${serverUser}@${serverHost} 'cd ${serverConfigPath} && docker stop h01front-nginx || true && docker rm h01front-nginx || true'"
    ...
    # 7. Start H01Front container
    Write-Host "[Step7] Starting H01Front container..." -ForegroundColor Cyan
    try {
        if ($plinkExists) {
    ...
    # 8. Verify deployment
    Write-Host "[Step8] Verifying H01Front deployment..." -ForegroundColor Cyan
    try {
        if ($plinkExists) {
            $plinkVerifyCommand = "plink -batch -pw '$serverPassword' ${serverUser}@${serverHost} 'docker ps | grep h01front-nginx'"
    ...
    Write-Host "[Done] H01Front microfront-end deployment successful!" -ForegroundColor Green
    Write-Host "[Access] http://$serverHost:3001" -ForegroundColor Yellow
